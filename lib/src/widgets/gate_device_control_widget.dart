...
class GateDeviceControlWidget extends StatefulWidget {
  ...
  @override
  State<GateDeviceControlWidget> createState() =>
      _GateDeviceControlWidgetState();
}
...
class _GateDeviceControlWidgetState extends State<GateDeviceControlWidget> {
  ...
  String _getGateDescription(int level) {
    ...
    switch (level) {
      ...
      case 25:
        return 'Mở 25% - Người đi bộ';
      case 50:
        return 'Mở 50% - Xe máy';
      case 75:
        return 'Mở 75% - Xe hơi nhỏ';
      case 100:
        return 'Mở hoàn toàn';
      default:
  ...
  void _controlGate(int targetLevel, StateSetter setDialogState) async {
    ...
    try {
      ...
      if (targetLevel == 0) {
        command = 'CLOSE';
        direction = _currentLevel > 0
            ? 'closing'
            : null; // Only closing if currently open
      } else {
